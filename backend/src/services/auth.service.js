...
const { error } = require("../dtos/mongoId.dto");
...
exports.getUser= async(data)=>{
   try {
     const { email, id } = data;
     const user= await User.findOne({$or:[{email:email},{id:id}]});
...
exports.UpdateUser= async(data)=>{
   try {
     const {first_name,last_name,email,password,adress,email1, id } = data;
     const user= await User.findOne({$or:[{email:email1},{id:id}]});
